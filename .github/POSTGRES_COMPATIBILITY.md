...
India Compliance targets **both MariaDB and PostgreSQL from a single codebase** (like Frappe
and ERPNext). The server test suite is run on both, but the PostgreSQL CI job
(`server-tests-postgres.yml`) is **label-gated** (it does not run on every PR — add the
`postgres` label to trigger it), so until it is required this guide is the always-on guard.
Greptile loads it as review context (`.greptile/config.json`).
