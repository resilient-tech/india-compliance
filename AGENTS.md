...
- **Write ZERO comments** — no one-liners, banners, TODOs, or docstrings. Leave
  the existing comments alone. The only exception: a docstring on a
  `@frappe.whitelist()` method, and only when I ask for it.
...
- Destructive ops (rm, force push, prod site, `bench migrate` on a live site) →
  stop
