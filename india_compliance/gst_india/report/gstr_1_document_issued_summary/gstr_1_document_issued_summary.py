...
def get_document_summary(filters, document_type, nature_of_document):
	...
	query = frappe.qb.from_(doctype).select(doctype.name, doctype.naming_series, doctype.creation, doctype.docstatus).where(
		doctype.company == filters.company
	).where(doctype.posting_date.between(filters.from_date, filters.to_date)).where(
		doctype.naming_series.isnotnull()
	).where(
		doctype.docstatus > 0
	)
	...
	for naming_series, name_data in naming_series_data.items():
		...
		if sorted_names and len(sorted_names[0]) > 0:
			res.append(frappe._dict({
				...
				"total_issued": name_data.get("canceled_count") + name_data.get("total_count")
			}))
...
def apply_document_wise_condition(filters, query, nature_of_document, doctype):
	...
	if nature_of_document == "Invoices for outward supply":
		...
		journal_entries = frappe.db.get_all("Journal Entry Account", filters={"account": ("in", bank_accounts), "debit": (">", 0),
			"credit": 0}, pluck = "parent")
		query = query.where(doctype.name.isin(journal_entries)).where(doctype.voucher_type != "Contra Entry")
