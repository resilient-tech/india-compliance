...
def setup_company_independent_fixtures(patch=False):
    ...
    frappe.enqueue(
        "india_compliance.gst_india.setup.add_hsn_sac_codes", now=frappe.flags.in_test
    )
...
def add_hsn_sac_codes():
    ...
    with open(
        frappe.get_app_path(
            "india_compliance", "gst_india", "data", "hsn_code_data.json"
        ),
        "r",
    ) as f:
    ...
    with open(
        frappe.get_app_path(
            "india_compliance", "gst_india", "data", "sac_code_data.json"
        ),
        "r",
    ) as f:
...
def make_property_setters(patch=False):
    ...
    journal_entry_types = frappe.get_meta("Journal Entry").get_options(
        "voucher_type"
    ).split("\n") + ["Reversal Of ITC"]
