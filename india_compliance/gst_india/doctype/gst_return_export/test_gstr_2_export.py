...
def _mock_names(raw):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch.object(GovReturnExporter, "_live_gstin_name", staticmethod(lambda gstin: MOCK_BUSINESS))
        )
        stack.enter_context(
            patch.object(
                GovReturnExporter, "_live_gstin_names", staticmethod(lambda gstin: (MOCK_LEGAL, MOCK_TRADE))
            )
        )
        yield stack
