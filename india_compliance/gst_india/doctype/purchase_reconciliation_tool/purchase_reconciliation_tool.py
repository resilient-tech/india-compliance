...
from india_compliance.gst_india.utils.inward_supply import (
    GSTRCategory,
    ReturnType,
    download_gstr_2a,
    download_gstr_2b,
...
class PurchaseReconciliationTool(Document):
    ...
    def on_update(self):
        ...
        for original, amended in ORIGINAL_VS_AMENDED.items():
            self.reconcile(original, amended)

        self.db_set("reconciliation_data", self.get_reconciliation_data())
    ...
    def guess_supplier_name(self, gstin):
        ...
        return self.gstin_party_map.setdefault(
            gstin, get_party_by_gstin(gstin) or "Unknown"
        )
