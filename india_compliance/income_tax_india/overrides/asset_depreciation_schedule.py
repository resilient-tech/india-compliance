...
def get_wdv_or_dd_depr_amount(
    ...
    if schedule_idx == 0:
        ...
        previous_schedule_date = add_months(schedule_date, -1 * cint(fb_row.frequency_of_depreciation))
        if is_last_day:
    ...
    if fb_row.frequency_of_depreciation == 12:
        ...
        if schedule_date < start_date_of_next_fiscal_year:
            depreciation_amount = flt(depreciable_value) * (flt(rate_of_depreciation) / 100) * fraction
        else:
            depreciation_amount = (
