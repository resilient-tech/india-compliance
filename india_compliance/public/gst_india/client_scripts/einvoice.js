export function setup_einvoice_actions(doctype) {
    frappe.ui.form.on(doctype, {
        async refresh(frm) {
            ...
            if (irn && ewaybill && !irn_cancelled && !eway_bill_cancelled) {
                const action = () => {
                    let message =
                        __("Cancellation of e-Waybill is currently not supported.") +
                        " ";
                    message += "<br><br>";
                    message += __(
                        "You must first use the portal to cancel the e-Waybill and then update the cancelled status in the ERPNext system."
                    );
