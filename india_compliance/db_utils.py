...
def bulk_update(
    ...
    :param docs: List of dictionaries to update
    ...
    Note:
        - Each dictionary should have a key `name` which is the docname of the document to update.
        - The rest of the keys are the fields to update.
    ...
    for index in range(0, total_docs, chunk_size):
        ...
        docs_map = {doc.pop("name"): doc for doc in chunk}
...
def build_query_and_update_in_bulk(doctype: str, docs: dict):
    ...
    for field in conditions:
        update_query = update_query.set(dt[field], conditions[field].else_(None))
